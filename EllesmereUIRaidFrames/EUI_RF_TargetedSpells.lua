...
local function ShowFor(caster, matches, texture, durObj, cdStart, cdDur, channeling)
    local raid = IsInRaid()
        ...
        if btn and btn:IsShown() then
            ...
            if icon then
                ...
                elseif cdStart and cdDur then
                    -- MoP path: cast fills up (reverse), channel depletes.
                    cd:SetReverse(not channeling)
                    cd:SetAlpha(1)
                    cd:SetCooldown(cdStart, cdDur)
                    cd:SetDrawSwipe(true)
                    cd:Show()
                else
                    cd:Clear()
...
local function Resolve(caster, myGen)
    ...
    local castName, _, texture, sMS, eMS = UnitCastingInfo(caster)
    local channeling = false
    if type(castName) == "nil" then
        castName, _, texture, sMS, eMS = UnitChannelInfo(caster)
        channeling = true
    end
    ...
    -- MoP swipe: no duration objects here, so drive the cooldown from the
    -- cast's start/end times (ms) returned by UnitCasting/ChannelInfo.
    local cdStart, cdDur
    if not durObj and type(sMS) == "number" and type(eMS) == "number" and eMS > sMS then
        cdStart = sMS / 1000
        cdDur = (eMS - sMS) / 1000
    end
    ...
    ShowFor(caster, matches, texture, durObj, cdStart, cdDur, channeling)
