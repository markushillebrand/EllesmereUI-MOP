...
local function ApplySpell(actor, key, subKey, spellName, spellID, amount, crit, over)
...
local function AddAmount(guid, name, class, key, subKey, spellName, spellID, amount, crit, over)
    if amount <= 0 then return end
    ApplySpell(GetOrCreateActor(overall, guid, name, class), key, subKey, spellName, spellID, amount, crit, over)
    if current then
        ApplySpell(GetOrCreateActor(current, guid, name, class), key, subKey, spellName, spellID, amount, crit, over)
    end
end
    ...
    if srcGroup then
        ...
        if guid then
            if isDamage and type(amount) == "number" then
                AddAmount(guid, name, class, "damage", "dmg", spellName, spellID, amount, critB)
            ...
            elseif isHeal and type(amount) == "number" then
                ...
                local eff = amount - ov
                if eff > 0 then
                    AddAmount(guid, name, class, "healing", "heal", spellName, spellID, eff, critB, ov)
                end
...
local ShowBarTooltip, OpenDetail, RefreshMeter, RefreshAll, CreateMeter, SpawnMeter, CloseMeter, ToggleSegMenu
...
local function CreateDetailBar(parent)
    ...
    bar:SetScript("OnEnter", function(self) if self._s then ShowDetailTooltip(self) end end)
...
local function RefreshDetail()
    ...
    detailWin.title:SetText((a and a.name) or "?")
    detailWin.seglabel:SetText(seg and SegmentLabel(seg) or "-")

    local list, tot = {}, 0
    if a then list, tot = ActorBreakdown(a, detailMode) end
    ...
    for i = 1, detailMaxBars do
        ...
        if e then
            ...
            bar:SetValue(tot > 0 and (v / tot) or 0)   -- row fill = share of segment total
...
local function CreateDetailWindow()
    detailWin = CreateFrame("Frame", "EllesmereUIDamageMeterDetail", UIParent)
    ...
    if EllesmereUI and EllesmereUI.PP and EllesmereUI.PP.CreateBorder then
        pcall(EllesmereUI.PP.CreateBorder, detailWin, 0, 0, 0, 1, 1, "OVERLAY", 7)
    end
...
local function CreateDeathWindow()
    deathWin = CreateFrame("Frame", "EllesmereUIDamageMeterDeaths", UIParent)
    ...
    if EllesmereUI and EllesmereUI.PP and EllesmereUI.PP.CreateBorder then
        pcall(EllesmereUI.PP.CreateBorder, deathWin, 0, 0, 0, 1, 1, "OVERLAY", 7)
    end
...
local function BuildModeMenu()
    modeMenu = CreateFrame("Frame", "EllesmereUIDamageMeterModeMenu", UIParent)
    ...
    if EllesmereUI and EllesmereUI.PP and EllesmereUI.PP.CreateBorder then
        pcall(EllesmereUI.PP.CreateBorder, modeMenu, 0, 0, 0, 1, 1, "OVERLAY", 7)
    end
...
function ToggleSegMenu(m, anchor)
    if not segMenu then
        segMenu = CreateFrame("Frame", "EllesmereUIDamageMeterSegMenu", UIParent)
        ...
        if EllesmereUI and EllesmereUI.PP and EllesmereUI.PP.CreateBorder then
            pcall(EllesmereUI.PP.CreateBorder, segMenu, 0, 0, 0, 1, 1, "OVERLAY", 7)
        end
...
function CreateMeter(p)
    ...
    local f = CreateFrame("Frame", "EllesmereUIDamageMeterFrame" .. meterFrameCount, UIParent)
    m.f = f
