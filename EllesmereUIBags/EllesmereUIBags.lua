...
local function CreateHeader()
    ...
    EllesmereUI.SafeAtlas(dice.icon, "charactercreate-icon-dice", "Interface\\Buttons\\UI-GroupLoot-Dice-Up")
    ...
    EllesmereUI.SafeAtlas(bagsBtn.icon, "bag-main", "Interface\\Icons\\INV_Misc_Bag_08")
    ...
    close.icon:SetTexture("Interface\\AddOns\\EllesmereUI-MoP\\media\\icons\\eui-close.png")
...
local function GetGoldTooltip()
    ...
    title:SetFont(GetFont(), 11, GetOutline())
    ...
    hint:SetFont(GetFont(), 10, GetOutline())
...
local function ShowGoldTooltip(anchor)
    ...
    if warbandGold then
        ...
        nameFS:SetText(EllesmereUI.L("|cffffcc80Warbank|r"))
    ...
    totalNameFS:SetText(EllesmereUI.L("|cffffcc80Total|r"))
...
local function RenderButton(btn, data, _, col, row, startX, currentY, _, interactiveEmpties)
    ...
    if not data.info then
        ...
        if data.itemLink then
            SetItemButtonQuality(btn, quality, data.itemLink, false, false)
        end
        ...
        if btn.icon and data.info and data.info.itemID then
            ...
            if canUse == nil then
                ...
                if IsEquippableItem(id) or C_Item.GetItemSpell(id) then
                    local tip = C_TooltipInfo and C_TooltipInfo.GetItemByID(id)
...
local function CreateSidebar()
    ...
    local ARROW_ICON = "Interface\\AddOns\\EllesmereUI-MoP\\media\\icons\\eui-arrow-left.png"
...
local function BuildSidebarButtons(categoryCounts, totalCount)
    ...
    for i, entry in ipairs(displayList) do
        ...
        if entry.isAtlas and C_Texture and C_Texture.GetAtlasInfo and not C_Texture.GetAtlasInfo(entry.icon) then
            -- MoP: atlas not present on this client; neutral fallback icon
...
function EUI_Bags:RefreshInventory()
    ...
    if selectedCategoryIndex == -1 then
        ...
        if not _warnHidden then
            ...
            warn:SetText(EllesmereUI.L("Changes made in OneBag will affect the positions of items in default Blizzard bags"))
            warn:Show()
            curY = curY - 14 - 5
        end
        ...
        mainHdr._label:SetText(EllesmereUI.L("Main Bags (") .. mainFilled .. " / " .. #mainSlots .. ")")
        mainHdr:Show()
        curY = curY - 22
        ...
        if #reagentSlotList > 0 then
            ...
            reagHdr._label:SetText(EllesmereUI.L("Reagent Bag (") .. reagFilled .. " / " .. #reagentSlotList .. ")")
            reagHdr:Show()
            curY = curY - 22
...
local function StartAddon()
    ...
    EUI_Bags.bg:SetTexture("Interface\\AddOns\\EllesmereUI-MoP\\media\\modern_blizz.png")
