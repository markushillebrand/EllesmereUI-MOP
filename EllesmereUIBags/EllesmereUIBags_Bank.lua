...
do
    local function MakeSecurePurchaseBtn(bankType)
        -- MoP: BankPanelPurchaseButtonScriptTemplate (Warband bank tab purchase)
        -- doesn't exist. Guard creation so the file can't abort loading; returns
        -- nil on clients lacking the template (all callers handle nil).
        local ok, b = pcall(CreateFrame, "Button", nil, sidebar, "BankPanelPurchaseButtonScriptTemplate")
...
function EUI_Bank:RefreshBank()
    ...
    local function RenderSlotContent(btn, bagID, slot, cachedInfo)
        ...
        if not info then
            ...
            if itemLink then SetItemButtonQuality(btn, quality, itemLink, false, false) end
...
function BuildBankSidebar()
    ...
    if _purchaseBtnWarband then _purchaseBtnWarband:Hide() end
...
-- MoP: Warband/account-bank-tab events are retail-only. Register safely
-- so invalid-event errors cannot abort setup (handlers never fire on MoP).
pcall(eventFrame.RegisterEvent, eventFrame, "BANK_TABS_CHANGED")
pcall(eventFrame.RegisterEvent, eventFrame, "BANK_TAB_SETTINGS_UPDATED")
