...
local QT_DEFAULTS = {
    profile = {
        questTracker = {
            ...
            -- Position (set via Unlock Mode; nil = Blizzard default).
            -- Stored CENTER/CENTER relative to UIParent to match the core
            -- unlock system's coordinate convention.
            trackerPos           = nil,
        },
    },
}
...
-- MoP Classic ships the legacy WatchFrame instead of the retail
-- ObjectiveTrackerFrame; both are base UI and load before us. Accept either
-- as the tracker so init proceeds on MoP.
if (_isLoaded and _isLoaded("Blizzard_ObjectiveTracker")) or _G.ObjectiveTrackerFrame or _G.WatchFrame then
    _sawOT = true
end
...
_G._EQT_RefreshAll = function()
    ...
    if EQT.ApplyTrackerPosition then EQT.ApplyTrackerPosition() end
end
