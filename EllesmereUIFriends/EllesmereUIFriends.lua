...
local CLASS_ICON_SPRITE_BASE = "Interface\\AddOns\\EllesmereUI-MoP\\media\\icons\\class-full\\"
...
local function SkinFriendButton(button)
    ...
    hover:SetAllPoints()
    EllesmereUI.SafeAtlas(hover, "groupfinder-highlightbar-green")
...
local function PostUpdateFriendButton(button)
    ...
    if not GetFFD(button).statusOrb then
        ...
        if _orbFile then
            ...
            EllesmereUI.SafeAtlas(GetFFD(button).statusOrb, "lootroll-animreveal-a")
...
local function SkinFriendsFrame()
    ...
    if not GetFFD(frame).sizeSet then
        ...
        local function ApplySize()
            ...
            _sizeApplied = true
            -- MoP: the classic FriendsFrame is narrower than retail's; the
            -- retail -40 shrink makes it too cramped, so widen it instead.
            ...
            -- Reposition the friends list scroll area into our list pane.
            -- Retail uses FriendsListFrame.ScrollBox; MoP Classic uses the
            -- HybridScrollFrame FriendsFrameFriendsScrollFrame instead.
            local listScroll = FriendsListFrame.ScrollBox or _G.FriendsFrameFriendsScrollFrame
            if listScroll then
                listScroll:ClearAllPoints()
                listScroll:SetPoint("TOPLEFT", frame, "TOPLEFT", LIST_LEFT, LIST_TOP)
                listScroll:SetPoint("BOTTOMRIGHT", frame, "BOTTOMRIGHT", LIST_RIGHT, LIST_BOTTOM)
            end
    ...
    local function SkinSubTabs()
        ...
        if lastSubTab then
            ...
            bcIcon:SetAllPoints()
            EllesmereUI.SafeAtlas(bcIcon, "voicechat-icon-textchat-silenced", "Interface\\Common\\VoiceChat-Muted")
    ...
    do -- search bar
        ...
        sPh:SetText(EllesmereUI.L("Invite Friend..."))
