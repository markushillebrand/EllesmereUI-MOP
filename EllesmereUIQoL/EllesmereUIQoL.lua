...
do
    ...
    local function UpdateSecondaryStats()
        ...
        -- MoP: Versatility (CR_VERSATILITY_DAMAGE_DONE) does not exist; guard.
        local hasVers = CR_VERSATILITY_DAMAGE_DONE and GetCombatRatingBonus and true
        local vers = 0
        if hasVers then
            local versRating = GetCombatRatingBonus(CR_VERSATILITY_DAMAGE_DONE) or 0
            local versBase = (GetVersatilityBonus and GetVersatilityBonus(CR_VERSATILITY_DAMAGE_DONE)) or 0
            local secret = issecretvalue and (issecretvalue(versRating) or issecretvalue(versBase))
            vers = secret and versRating or (versRating + versBase)
        end
...
do
    ...
    local function CreateDurabilityWarning()
        ...
        durWarnOverlay._show = function(pct)
            ApplySettings()
            durWarnOverlay._text:SetText(EllesmereUI.L("Low Durability (") .. math.floor(pct) .. "%)")
            durWarnOverlay:Show()
            ag:Play()
        end
