...
local function Font()  return (EllesmereUI and EllesmereUI.GetFontPath and EllesmereUI.GetFontPath()) or STANDARD_TEXT_FONT end
local function L(s)    return (EllesmereUI and EllesmereUI.L and EllesmereUI.L(s)) or s end
local function Green() return (EllesmereUI and EllesmereUI.ELLESMERE_GREEN) or { r = 0.05, g = 0.82, b = 0.62 } end
...
local function AddButton(parent, label, w)
    ...
    if EllesmereUI and EllesmereUI.PP and EllesmereUI.PP.CreateBorder then
        EllesmereUI.PP.CreateBorder(b, 1, 1, 1, EllesmereUI.DD_BRD_A or 0.2, 1)
    end
...
local function MakeWindow(globalName, titleText, w, h)
    ...
    if EllesmereUI and EllesmereUI.PP and EllesmereUI.PP.CreateBorder then
        EllesmereUI.PP.CreateBorder(f, 1, 1, 1, EllesmereUI.DD_BRD_A or 0.2, 1)
    end

    local title = f:CreateFontString(nil, "OVERLAY")
    title:SetFont(Font(), 15, "")
    local g = Green(); title:SetTextColor(g.r, g.g, g.b)
...
function ns.LR_ToggleSession()
    if not sessionWin then
        sessionWin = MakeWindow("EUI_LootRoll_SessionWindow", L("Loot Roll Session"), 460, 380)
...
function ns.LR_ToggleBonusHistory()
    if not bonusWin then
        bonusWin = MakeWindow("EUI_LootRoll_BonusWindow", L("Bonus Roll History"), 460, 380)
...
function ns.LR_ToggleWishlist()
    if not wishWin then
        wishWin = MakeWindow("EUI_LootRoll_WishlistWindow", L("Loot Wishlist"), 480, 420)
...
function ns.LR_ToggleWishlistBrowser()
    if not browserWin then
        browserWin = MakeWindow("EUI_LootRoll_WishlistBrowser", L("Loot Wishlist"), 520, 470)
...
local function AnchorMinimapButton(b)
    b:ClearAllPoints()
    local q = EllesmereUI and EllesmereUI._minimapQueueBtn
    local cal = EllesmereUI and EllesmereUI._minimapCalendarBtn
...
local function CreateMinimapButton()
    ...
    local b = CreateFrame("Button", "EUI_LootRoll_MinimapButton", UIParent)
    ...
    -- so all EllesmereUI minimap buttons share one consistent look).
    local bg = b:CreateTexture(nil, "BACKGROUND")
    bg:SetAllPoints()
...
--  Slash shortcuts
-------------------------------------------------------------------------------
SLASH_EUILRWIN1 = "/euiloot"
SlashCmdList["EUILRWIN"] = function(msg)
