...
--  History is stored per-character in EllesmereUILootRollCharDB (declared
...
-------------------------------------------------------------------------------
local function CDB()
    if type(EllesmereUILootRollCharDB) ~= "table" then EllesmereUILootRollCharDB = {} end
    local c = EllesmereUILootRollCharDB
...
function ns.LR_Announce(msg, sub)
    ...
    if sub then print("|cff0cd29fEllesmereUI:|r |cffaaaaaa" .. sub .. "|r") end
...
local function HandleBonusPrompt(spellID, boss)
    ...
    if s.bonusReminder and hasMatch then
        ...
        local head = boss
            and EllesmereUI.Lf("Bonus roll: wish item at %1$s!", boss)
...
-------------------------------------------------------------------------------
SLASH_EUILRBONUS1 = "/euibonus"
SlashCmdList["EUILRBONUS"] = function(msg)
    msg = (msg or ""):lower():gsub("^%s+", ""):gsub("%s+$", "")
    local p = function(s) print("|cff0cd29fEllesmereUI BonusRoll:|r " .. s) end
    if msg == "clear" then
    ...
    if msg == "test" then
        ...
        Record("money", nil, 0)
        p("added 2 fake entries (1 item, 1 gold).")
        return
    end
    local hist = History()
