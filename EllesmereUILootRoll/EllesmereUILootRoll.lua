...
local function EnsureDB()
    ...
    if not (EllesmereUI and EllesmereUI.Lite) then return nil end
    _db = EllesmereUI.Lite.NewDB("EllesmereUILootRollDB", DEFAULTS)
    _G._ELR_DB = _db
    return _db
end
local function DB()
...
local function FontPath()
    return (EllesmereUI and EllesmereUI.GetFontPath and EllesmereUI.GetFontPath()) or STANDARD_TEXT_FONT
end
...
local function EnsureAnchor()
    ...
    anchor = CreateFrame("Frame", "EllesmereUILootRollAnchor", UIParent)
    ApplyAnchor()
    return anchor
end
...
local function RollBtn_OnClick(self)
    local bar = self._bar
    if bar and bar.rollID and not bar._sim then RollOnLoot(bar.rollID, self._rolltype) end
end
...
local function HandleStartRoll(rollID, rollTime)
    ...
    bar.bind:SetText(bop and "BoP" or "BoE")
    ...
    EllesmereUI:RegisterUnlockElements({
        MK({
            ...
            savePos = function(_, point, relPoint, x, y)
                ...
                if not EllesmereUI._unlockActive then ApplyAnchor() end
            end,
...
function ELR.StartSim()
    ...
    bar.name:SetTextColor(q.r, q.g, q.b)
    SetBorderColor(bar, q.r, q.g, q.b, 1)
    bar.status:SetStatusBarColor(q.r, q.g, q.b, 0.45)
    bar.status:SetMinMaxValues(0, 30000)
    bar.status:SetValue(30000)
    bar.bind:SetText("BoP"); bar.bind:SetTextColor(1, 0.3, 0.1)
    ...
    print("|cff66ccffEUI-LR|r Sim started. /euilr stop to end.")
end
...
SLASH_EUILR1 = "/euilr"
SlashCmdList["EUILR"] = function(msg)
    msg = (msg or ""):lower():gsub("^%s+", ""):gsub("%s+$", "")
    if msg == "test" or msg == "sim" then
        ELR.StartSim()
    elseif msg == "stop" or msg == "off" then
        ELR.StopSim()
        print("|cff66ccffEUI-LR|r Sim stopped.")
    else
        print("|cff66ccffEUI-LR|r Commands: /euilr test | stop")
    end
end
