...
function ns.LR_RollLabel(rt)
    local key = (rt == ROLL_NEED and "Need")
        ...
        or "?"
    return (EllesmereUI and EllesmereUI.L and EllesmereUI.L(key)) or key
end
...
-------------------------------------------------------------------------------
SLASH_EUILRSESSION1 = "/euisession"
SlashCmdList["EUILRSESSION"] = function(msg)
    msg = (msg or ""):lower():gsub("^%s+", ""):gsub("%s+$", "")
    local p = function(s) print("|cff0cd29fEllesmereUI Session:|r " .. s) end
    if msg == "clear" then ns.LR_ClearSession(); p("session cleared."); return end
    if #session == 0 then p("no completed rolls recorded this session."); return end
    p(("%d completed roll(s) this session:"):format(#session))
