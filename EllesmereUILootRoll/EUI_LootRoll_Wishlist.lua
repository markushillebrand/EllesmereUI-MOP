...
--
--  Data is per-character in EllesmereUILootRollCharDB.wishlist.
-------------------------------------------------------------------------------
...
ns.LR_T = ns.LR_T or function(s)
    return (EllesmereUI and EllesmereUI.L and EllesmereUI.L(s)) or s
end
...
--  Debug
-------------------------------------------------------------------------------
SLASH_EUIWISH1 = "/euiwish"
SlashCmdList["EUIWISH"] = function(msg)
    msg = (msg or ""):lower():gsub("^%s+", ""):gsub("%s+$", "")
    local p = function(s) print("|cff0cd29fEllesmereUI Wishlist:|r " .. s) end
    if msg == "scan" then ScanBags(); p("bag scan done."); return end
    if msg == "ejdump" then
        EnsureEJ()
