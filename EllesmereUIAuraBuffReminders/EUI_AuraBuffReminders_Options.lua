    ...
    local function BuildTalentRemindersPage(pageName, parent, yOffset)
        ...
        local function GetAllTalents()
            ...
            if not configID then
                -- MoP tiered talents: C_ClassTalents/C_Traits don't exist here.
                -- Enumerate the talent tiers (x3 columns) via the MoP-native
                -- C_SpecializationInfo.GetTalentInfo struct API.
                if C_SpecializationInfo and C_SpecializationInfo.GetTalentInfo then
                    local group = (GetActiveSpecGroup and GetActiveSpecGroup()) or 1
                    local seen = {}
                    for tier = 1, 7 do
                        for column = 1, 3 do
                            local info = C_SpecializationInfo.GetTalentInfo({ tier = tier, column = column, groupIndex = group, target = "player" })
                            if info and info.spellID and info.spellID > 0 and info.name and info.name ~= ""
                                and not seen[info.spellID] then
                                seen[info.spellID] = true
                                talents[#talents + 1] = { spellID = info.spellID, name = info.name }
                            end
                        end
                    end
