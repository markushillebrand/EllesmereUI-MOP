...
local function PreSkinCharacterSheet()
    ...
    bg:SetTexture("Interface\\AddOns\\EllesmereUI-MoP\\media\\modern_blizz.png")
    ...
    do local _r = select(16, _G.CharacterMainHandSlot:GetRegions()); if _r and _r.SetTexCoord then _r:SetTexCoord(.8,.8,.8,.8,.8,.8,.8,.8) end end
    do local _r = select(17, _G.CharacterMainHandSlot:GetRegions()); if _r and _r.SetTexCoord then _r:SetTexCoord(.8,.8,.8,.8,.8,.8,.8,.8) end end
    do local _r = select(16, _G.CharacterSecondaryHandSlot:GetRegions()); if _r and _r.SetTexCoord then _r:SetTexCoord(.8,.8,.8,.8,.8,.8,.8,.8) end end
    do local _r = select(17, _G.CharacterSecondaryHandSlot:GetRegions()); if _r and _r.SetTexCoord then _r:SetTexCoord(.8,.8,.8,.8,.8,.8,.8,.8) end end
    ...
    do local _r = select(16, _G.CharacterMainHandSlot:GetRegions()); if _r and _r.SetTexCoord then _r:SetTexCoord(0.8, 0.8, 0.8, 0.8, 0.8, 0.8, 0.8, 0.8) end end
    do local _r = select(17, _G.CharacterMainHandSlot:GetRegions()); if _r and _r.SetTexCoord then _r:SetTexCoord(0.8, 0.8, 0.8, 0.8, 0.8, 0.8, 0.8, 0.8) end end
    do local _r = select(16, _G.CharacterSecondaryHandSlot:GetRegions()); if _r and _r.SetTexCoord then _r:SetTexCoord(0.8, 0.8, 0.8, 0.8, 0.8, 0.8, 0.8, 0.8) end end
    do local _r = select(17, _G.CharacterSecondaryHandSlot:GetRegions()); if _r and _r.SetTexCoord then _r:SetTexCoord(0.8, 0.8, 0.8, 0.8, 0.8, 0.8, 0.8, 0.8) end end
...
local function SkinCharacterSheet()
    ...
    local function ApplyTabVisibility(isCharacterTab)
        UpdateTabVisuals()
        -- MoP: the right-side stats panel follows the frame's expand toggle
        -- (CharacterFrame.Expanded), matching Blizzard's collapse/expand button.
        -- On retail the frame is always expanded, so this mirrors that when set.
        local showStats = isCharacterTab and (CharacterFrame and CharacterFrame.Expanded == true)
        ...
        if GetFFD(frame).statsPanel       then GetFFD(frame).statsPanel:SetShown(showStats)            end
        if GetFFD(frame).iLvlText         then GetFFD(frame).iLvlText:SetShown(showStats)              end
        if GetFFD(frame).sidebarBgFrame   then GetFFD(frame).sidebarBgFrame:SetShown(showStats)        end
        if GetFFD(frame).scrollFrame      then GetFFD(frame).scrollFrame:SetShown(showStats)           end
        if GetFFD(frame).scrollBar        then GetFFD(frame).scrollBar:SetShown(showStats)             end
        ...
        if GetFFD(frame).statsSections then
            for _, sectionData in ipairs(GetFFD(frame).statsSections) do
                if sectionData.container then
                    sectionData.container:SetShown(showStats)
                end
            end
        end
    ...
    -- MoP: re-evaluate stats-panel visibility when the frame is expanded or
    -- collapsed (CharacterFrameMixin:Expand/:Collapse). Retail has no toggle,
    -- so these hooks simply never fire there.
    if CharacterFrame and CharacterFrame.Expand and CharacterFrame.Collapse then
        local function _refreshExpand() ApplyTabVisibility((frame.selectedTab or 1) == 1) end
        hooksecurefunc(CharacterFrame, "Expand", _refreshExpand)
        hooksecurefunc(CharacterFrame, "Collapse", _refreshExpand)
        -- Bulletproof: react to the user clicking the expand/collapse button
        -- directly. The mixin-method hooks above don't fire on every code path
        -- (cached references / toggle wrappers), but the button OnClick always
        -- does. Defer one frame so CharacterFrame.Expanded is settled.
        if CharacterFrameExpandButton then
            CharacterFrameExpandButton:HookScript("OnClick", function()
                if C_Timer and C_Timer.After then
                    C_Timer.After(0, _refreshExpand)
                else
                    _refreshExpand()
                end
            end)
    ...
    for sectionIdx, section in ipairs(statSections) do
        ...
        do
            local arrowSize = 12
            local MEDIA = "Interface\\AddOns\\EllesmereUI-MoP\\media\\"
    ...
    frame:HookScript("OnShow", function()
        -- pcall each: HONOR_XP_UPDATE / HONOR_LEVEL_UPDATE (Legion+) are
        -- unknown on MoP and would otherwise throw on RegisterEvent.
        for _, ev in ipairs(_STATS_EVENTS) do pcall(statsEventFrame.RegisterEvent, statsEventFrame, ev) end
    end)
    frame:HookScript("OnHide", function()
    ...
    RefreshEquipmentSets = function()
        ...
        local function _acquireTile(index)
            ...
            cogTex:SetTexture("Interface\\AddOns\\EllesmereUI-MoP\\media\\icons\\cogs-3.png")
    ...
    do
        local EYE_VISIBLE   = "Interface\\AddOns\\EllesmereUI-MoP\\media\\icons\\eui-visible.png"
        local EYE_INVISIBLE = "Interface\\AddOns\\EllesmereUI-MoP\\media\\icons\\eui-invisible.png"
        local hidden = false
